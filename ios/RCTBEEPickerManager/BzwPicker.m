...
-(void)makeuiWith:(NSArray *)topbgColor With:(NSArray *)bottombgColor With:(NSArray *)leftbtnbgColor With:(NSArray *)rightbtnbgColor With:(NSArray *)centerbtnColor
{
    ...
    _cenLabel=[[UILabel alloc]initWithFrame:CGRectMake(90, 5, SCREEN_WIDTH-180, 30)];

    _cenLabel.textAlignment=NSTextAlignmentCenter;

    [_cenLabel setFont:[UIFont systemFontOfSize:[_pickerToolBarFontSize integerValue]]];

    _cenLabel.text=self.centStr;

    [_cenLabel setTextColor:[self colorWith:centerbtnColor]];

    [view addSubview:_cenLabel];
